/**
 * Client-side food image dictionary for mobile.
 * Fallback when API doesn't return foodItems with imageUrl.
 */
...
/**
 * Parse food items from text response. Fallback when API doesn't return foodItems.
 */
export function parseFoodItemsFromText(content: string): { name: string; calories: number; imageUrl?: string }[] | undefined {
  ...
  for (const line of lines) {
    ...
    if (match) {
      items.push({ name: match[1].trim(), calories: parseInt(match[2], 10), imageUrl: getFoodImageUrl(match[1].trim()) });
    }
  }
